...
def _dropbox_headers() -> dict:
    if not DROPBOX_ACCESS_TOKEN:
        raise HTTPException(
            status_code=503,
            detail="DROPBOX_ACCESS_TOKEN ontbreekt op de server."
        )
...
async def _store_quote_files(conn, quote_id: str, files: list[UploadFile]) -> None:
    ...
    for upload in files or []:
        ...
        # Dropbox gebruikt overwrite: een gewijzigde PDF/STEP met dezelfde naam
        # vervangt dus netjes de vorige versie in dezelfde offertemap.
