...
def _assembly_cache_path(job_id: str) -> Path:
    return CACHE_DIR / job_id / "assembly_mesh_physical_cut_v60.json"
...
def _physical_cut_polylines(
    ...
    """
    Return (base end contours, extra machining contours), on OUTER skin only.

    - inner wall loops never enter the result
    - wall-thickness connector edges never enter the result
    - both straight and curved outside radii remain intact
    - two end contours are base cuts
    - holes/slots/notches away from the two main ends are extra machining
    """
    if not detail or not detail.get("recognized"):
        return [],[]
    ...
    components=_connected_edge_components(candidates,tol=0.16)
    _,_,_,_,ow,oh,half_len,*_=basis

    summaries=[]
    for comp in components:
        total_len=sum(float(e["length"]) for e in comp)
        weighted_z=sum(float(e["zmean"])*max(float(e["length"]),1e-6) for e in comp)/max(total_len,1e-6)
        summaries.append({
            "edges":comp,
            "length":total_len,
            "zmean":weighted_z,
    ...
    # Main end contour can span substantially in Z for a mitre. Select the
    # longest connected outside contour in a generous zone at each extreme.
    transverse=max(ow,oh,1.0)
    end_zone=max(transverse*1.75,float(basis[-1])*0.08,4.0)
    plus=[
    ...
    base_components=[]
    if plus:
        base_components.append(max(plus,key=lambda x:x["length"]))
    if minus:
        pick=max(minus,key=lambda x:x["length"])
        if pick not in base_components:
            base_components.append(pick)

    # Very short/odd body fallback: choose the two components furthest apart in
    # longitudinal position, provided they are substantial contours.
    if not base_components and summaries:
        ordered=sorted(summaries,key=lambda x:x["zmean"])
        base_components=[ordered[0]]
        if len(ordered)>1:
            base_components.append(ordered[-1])

    base_ids={id(c) for c in base_components}
    base_lines=[]
    feature_lines=[]
    ...
    return base_lines,feature_lines
...
def _feature_polylines(
    ...
    _base,features=_physical_cut_polylines(solid,detail)
    return features
...
def _base_cut_polylines(
    ...
    base,_features=_physical_cut_polylines(solid,detail)
    return base
...
def assembly_mesh(job_id: str):
    ...
    try:
        ...
        for solid_index, solid in enumerate(solids, start=1):
            ...
            base_cut_lines, feature_lines = _physical_cut_polylines(solid, detail)
...
def solid_mesh(job_id: str, solid_index: int):
    ...
    try:
        ...
        raw_base_cut_lines, raw_feature_lines = _physical_cut_polylines(solid, detail)
