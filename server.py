...
def _assembly_cache_path(job_id: str) -> Path:
    return CACHE_DIR / job_id / "assembly_mesh_features_v39.json"
...
def assembly_mesh(job_id: str):
    ...
    try:
        ...
        for solid_index, solid in enumerate(solids, start=1):
            ...
            # Preserve the real STEP body and all machining features.
            # Only material outside the proven net-length interval is clipped.
            visible_shape, was_trimmed, raw_length, net_length = _clip_solid_to_net_length(
                solid, detail
            )
...
def solid_mesh(job_id: str, solid_index: int):
    ...
    try:
        ...
        # Loose-part viewer uses the same original STEP geometry with only
        # the trimmed end material removed.
        visible_shape, was_trimmed, raw_length, net_length = _clip_solid_to_net_length(
            solid, detail
        )
