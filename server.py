...
def eboekhouden_relation_search(q: str="", limit: int=8):
    query=str(q or "").strip()
    if len(query)<2: return {"ok":True,"relations":[]}
    rows=_eboek_find_relations(name=query,limit=max(1,min(20,limit)))
    # If name search is strict and empty, try e-mail when query looks like mail.
    if not rows and "@" in query:
        rows=_eboek_find_relations(email=query,limit=max(1,min(20,limit)))
    return {"ok":True,"relations":[_eboek_relation_public(r) for r in rows]}
