...
def _feature_polylines(
    ...
    """Return only credible EXTRA laser-cut contours.

    Normal tube geometry (longitudinal edges, corner radii, end perimeter and
    STEP face-split/seam edges) must never become machining length.  Extra
    machining is accepted conservatively as a CLOSED connected contour made
    from non-standard edges. This matches holes, slots and closed cut-outs and
    prevents hundreds of ordinary BREP edges from inflating cut time.
    """
    if not detail or not detail.get("recognized"):
        return []
    profile_type = str(detail.get("type") or "").lower()
    ...
    try:
        for edge in solid.Edges():
            ...
            if len(pts) < 2:
                continue
            # Never accept a near-longitudinal construction/split edge as an
            # extra operation, even if STEP topology split it into pieces.
            arr = np.array(pts, dtype=float)
            # v55: basis = (center, xdir, ydir, Z/lengte-as, breedte, hoogte, halve_lengte).
            # De vorige versie gebruikte hier per ongeluk ydir en hoogte, waardoor
            # geldige gaten en uitsparingen soms uit de feature-lijst verdwenen.
            ...
            # Alleen bijna-volledige lengtenaden uitsluiten; langwerpige sleuven
            # en uitsparingen zijn geldige laserbewerkingen.
            if axial_span > max(100.0, member_len * 0.92):
                continue
    ...
    # Cluster endpoints. A true hole/slot/cut-out contour has degree 2 at all
    # vertices (or is itself a closed curve). Open seam/split fragments are
    # discarded. Tolerance is intentionally small relative to tube dimensions.
    tol = 0.35
    def key(p):
    ...
    for i, pts in enumerate(candidates):
        endpoints.append((key(pts[0]), key(pts[-1])))

    node_edges = {}
    for i, (a, b) in enumerate(endpoints):
        node_edges.setdefault(a, []).append(i)
        node_edges.setdefault(b, []).append(i)
    ...
    for i, pts in enumerate(candidates):
        a, b = endpoints[i]
        node_point.setdefault(a, pts[0])
        node_point.setdefault(b, pts[-1])

    c, xdir, ydir, zdir, ow, oh, half_len = basis
    hpw, hph = ow / 2.0, oh / 2.0
    boundary_tol = max(0.65, min(ow, oh) * 0.055)
    end_tol = max(0.8, max(1.0, half_len * 2.0) * 0.003)

    def near_profile_boundary(pt):
        try:
            p = np.array(pt, dtype=float) - c
            x = float(np.dot(p, xdir))
            y = float(np.dot(p, ydir))
            z = float(np.dot(p, zdir))
            return (
                abs(abs(x) - hpw) <= boundary_tol
                or abs(abs(y) - hph) <= boundary_tol
                or abs(abs(z) - half_len) <= end_tol
            )
        except Exception:
            return False
    ...
    for seed in range(len(candidates)):
        ...
        while stack:
            ...
            if i in comp:
                continue
            comp.add(i); seen.add(i)
            a, b = endpoints[i]
            nodes.update((a, b))
            for n in (a, b):
                for j in node_edges.get(n, []):
                    if j not in comp:
                        stack.append(j)
        ...
        odd_nodes = [n for n, deg in degrees.items() if deg % 2]
        closed = bool(nodes) and not odd_nodes and all(deg >= 2 for deg in degrees.values())
        if closed:
            accepted.update(comp)
            continue

        # Open notch/cut-out: precies twee open einden die beide aansluiten
        # op een normale buitenrand of op het uiteinde van het profiel.
        if len(odd_nodes) == 2:
            comp_len = sum(_polyline_length_mm(candidates[i]) for i in comp)
            max_reasonable = max(ow + oh + 80.0, (half_len * 2.0) * 0.85)
            endpoints_on_boundary = all(near_profile_boundary(node_point.get(n)) for n in odd_nodes)
            if comp_len >= 0.8 and comp_len <= max_reasonable and endpoints_on_boundary:
                accepted.update(comp)

    return [candidates[i] for i in sorted(accepted)]
