...
def _profile_basis_for_features(
    ...
    # Determine the nominal corner-radius centre offset directly from the STEP.
    # For a rounded rectangular tube, outer- and inner-radius arcs share the
    # same corner centre. Example 40x20 R1.75: centres are near ±18.25/±8.25.
    # This lets us recognise profile radii even when a notch intersects them,
    # without confusing ordinary drilled holes with profile geometry.
    hpw = ow / 2.0
    hph = oh / 2.0
    corner_offsets: list[float] = []

    if hpw > 0.0 and hph > 0.0:
        max_corner_offset = max(0.8, min(ow, oh) * 0.22)
        equal_tol = max(0.45, min(ow, oh) * 0.025)

        for edge in solid.Edges():
            if str(edge.geomType() or "").upper() != "CIRCLE":
                continue
            try:
                r = float(edge.radius())
                center = np.array(edge.arcCenter().toTuple(), dtype=float) - c
            except Exception:
                continue

            if r <= 1e-5 or r > min(ow, oh) * 0.24:
                continue

            cx = abs(float(np.dot(center, xdir)))
            cy = abs(float(np.dot(center, ydir)))
            dx = hpw - cx
            dy = hph - cy

            # A true profile corner has the arc centre inset by essentially
            # the same amount from both outer profile planes.
            if (
                -equal_tol <= dx <= max_corner_offset
                and -equal_tol <= dy <= max_corner_offset
                and abs(dx - dy) <= equal_tol
            ):
                corner_offsets.append(max(0.0, (dx + dy) / 2.0))

    corner_offset = float(np.median(corner_offsets)) if corner_offsets else 0.0

    return c, xdir, ydir, zdir, ow, oh, half_len, corner_offset


def _is_standard_profile_edge(
    edge: cq.Shape,
    basis: tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float, float, float, float],
) -> bool:
    """
    Return True ONLY for geometry that belongs to the purchased stock profile.

    General rule (dimension independent):
    - genuine longitudinal profile seams / tangent edges are stock geometry;
    - the profile's own rounded corner arcs are stock geometry everywhere;
    - every other edge is machining/cut geometry: straight or mitred ends,
      notches, slots, holes, tabs and other cut-outs.

    This deliberately does NOT try to recognise an "end plane". A slanted,
    stepped or notched end has no single constant-Z plane, so end-plane based
    logic inevitably loses parts of the contour.
    """
    c, xdir, ydir, zdir, ow, oh, half_len, corner_offset = basis

    if ow <= 0.0 or oh <= 0.0:
        # Unknown profile: do not suppress possible machining.
        return False
    ...
    # The purchased profile radius is never a laser cut. Detect it by the
    # radius centre being inset equally from both outer profile planes. This
    # applies at ordinary ends as well: the tiny corner-radius segment itself
    # is profile geometry and stays unhighlighted, while all adjoining flats
    # of the end contour remain highlighted.
    if gt == "CIRCLE":
        try:
            r = float(edge.radius())
            center = np.array(edge.arcCenter().toTuple(), dtype=float) - c
            ...
            centre_tol = max(tol_xy, min(ow, oh) * 0.04)
            max_r = min(ow, oh) * 0.30

            if 1e-5 < r <= max_r:
                # Prefer the measured nominal centre offset when available.
                if corner_offset > 0.0:
                    if (
                        abs(dx - corner_offset) <= centre_tol
                        and abs(dy - corner_offset) <= centre_tol
                    ):
                        return True
                # Robust fallback for STEP files where only part of the radius
                # family survived topology splitting.
                elif (
                    -centre_tol <= dx <= max_r + centre_tol
                    and -centre_tol <= dy <= max_r + centre_tol
                    and abs(dx - dy) <= centre_tol
                ):
                    return True
        except Exception:
            pass

        # Other circular geometry is a drilled hole / circular cut.
        return False

    # Non-circular curves are machining by default. This keeps slots and shaped
    # cut-outs. (Normal tube corner radii from STEP are CIRCLE arcs.)
    if gt in {"ELLIPSE", "BSPLINE", "BEZIER"}:
        return False
    ...
    dz_span = max(zs) - min(zs)

    if gt == "LINE":
        # Suppress only genuine longitudinal stock seams/tangent lines. They
        # must be very closely parallel to the member axis AND live in the
        # profile corner/tangent band. A notch wall can also be longitudinal,
        # but lies on a flat face away from both corner bands and is retained.
        vec = np.array([
            xs[-1] - xs[0],
            ys[-1] - ys[0],
            zs[-1] - zs[0],
        ...
        if length > 1e-7:
            axis_ratio = abs(float(vec[2])) / length
        else:
            axis_ratio = 0.0
        ...
        if axis_ratio >= 0.9985:
            corner_band = max(
                tol_xy * 1.35,
                (corner_offset + tol_xy * 1.10) if corner_offset > 0.0 else tol_xy * 1.75,
            )
            near_corner_x = all((hpw - abs(x)) <= corner_band for x in xs)
            near_corner_y = all((hph - abs(y)) <= corner_band for y in ys)
            if near_corner_x and near_corner_y:
                return True

        # ALL other straight edges are cut geometry. This is the key to full
        # contours on straight, mitred, stepped and notched tube ends.
        return False
