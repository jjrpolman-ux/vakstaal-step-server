...
def assembly_mesh(job_id: str):
    """
    Return the complete STEP assembly as separate meshes while preserving
    original STEP coordinates. This allows the browser to highlight selected
    solids without losing the assembled position/orientation.
    """
    cleanup_old_jobs()
    ...
    try:
        ...
        for solid_index, solid in enumerate(solids, start=1):
            vertices, triangles = solid.tessellate(0.9)
