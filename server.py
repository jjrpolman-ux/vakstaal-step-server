...
def _eboek_relation_public(rel):
    return {
        ...
        "phoneNumber":rel.get("phoneNumber") or rel.get("phone_number") or "",
        "mobilePhoneNumber":rel.get("mobilePhoneNumber") or rel.get("mobile_phone_number") or "",
...
def _eboek_find_relations(name="", email="", limit=20):
...
def eboekhouden_relation_search(q: str="", limit: int=10):
    ...
    rows=_eboek_find_relations(name=query,limit=max(wanted,20))

    if not rows and "@" in query:
        rows=_eboek_find_relations(email=query,limit=max(wanted,20))

    # Sommige e-Boekhouden-accounts/API-versies zoeken op naam vrij strikt.
    # In dat geval halen we een beperkte relationele set op en rangschikken
    # we lokaal op "begint met" en daarna "bevat".
    if not rows:
        try:
            broad=_eboek_http("GET","/v1/relation",query={"limit":200,"offset":0})
            all_rows=_eboek_items(broad)
            needle=query.casefold()
            starts=[]
            contains=[]
            for relation in all_rows:
                name=str(relation.get("name") or "").strip()
                code=str(relation.get("code") or "").strip()
                email=str(
                    relation.get("emailAddress")
                    or relation.get("email_address")
                    or ""
                ).strip()
                searchable=" ".join([name,code,email]).casefold()
                if not searchable:
                    continue
    ...
    # Duplicaten op relatie-ID eruit en naam-voorkeur behouden.
    unique=[]
    seen=set()
    for relation in rows:
        key=str(relation.get("id") or relation.get("code") or relation.get("name") or "")
        if key in seen:
            continue
        seen.add(key)
