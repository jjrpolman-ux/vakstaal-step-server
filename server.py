...
# v53: radius-aware normal-profile edge filtering for snijlijnen
...
def _feature_polylines(
    ...
    try:
        for edge in solid.Edges():
            ...
            axis = np.array(basis[2], dtype=float)
            proj = arr @ axis
            axial_span = float(proj.max() - proj.min()) if len(proj) else 0.0
            member_len = max(1.0, float(basis[5]))
            if axial_span > max(25.0, member_len * 0.18):
                continue
    ...
    for seed in range(len(candidates)):
        ...
        # Closed component: every endpoint node has even degree within this
        # component. Single closed curves naturally have identical end keys.
        closed = True
        for n in nodes:
            deg = 0
            for i in comp:
