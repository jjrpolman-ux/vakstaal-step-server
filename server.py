...
STEP_MATERIAL_LENGTH_VERSION = 5  # longest main wall side
...
def _physical_material_length_along_axis(
    ...
    if candidates:
        ...
        if spans:
            return max(spans),axis,"longest-main-wall-side-v5"
    ...
    if straight_lengths:
        return max(straight_lengths),axis,"longest-axis-edge-fallback-v5"

    return max(0.0,float(analyzer_length or 0.0)),axis,f"{analyzer_method}-fallback-v5"
...
def _assembly_cache_path(job_id: str) -> Path:
    return CACHE_DIR / job_id / "assembly_mesh_physical_cut_v64_main_wall_length.json"
