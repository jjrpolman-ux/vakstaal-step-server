...
def _dropbox_rpc(endpoint: str, payload: dict) -> dict:
    if not DROPBOX_ACCESS_TOKEN:
        raise HTTPException(status_code=503, detail="DROPBOX_ACCESS_TOKEN ontbreekt op de server.")
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Dropbox API fout ({exc.code}): {detail[:500]}") from exc
...
def _dropbox_upload_bytes(path: str, data: bytes) -> dict:
    ...
    try:
        ...
        detail=exc.read().decode("utf-8", errors="replace")
        raise HTTPException(status_code=502, detail=f"Dropbox uploadfout ({exc.code}): {detail[:500]}") from exc
...
def dropbox_status():
    ...
    return {"ok":True,"configured":True,"dropbox_response":result}
