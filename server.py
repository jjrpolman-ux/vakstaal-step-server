...
def _assembly_cache_path(job_id: str) -> Path:
    return CACHE_DIR / job_id / "assembly_mesh_feature_accent_v45.json"
...
def _is_standard_profile_edge(
    ...
    True = ordinary tube/profile geometry.
    False = likely extra machining geometry (hole/slot/notch/tab/lip/cut-out).

    It deliberately prefers false negatives over false positives, so a plain
    profile stays visually standard.
    """
    if not detail or not detail.get("recognized"):
        return True
    ...
    # Circular/elliptical edges are never part of a standard rectangular tube.
    ...
    c, xdir, ydir, zdir, ow, oh, half_len = _profile_basis_for_features(solid, detail)
    if ow <= 0.0 or oh <= 0.0:
        return True
    ...
    # Standard long profile edges lie on outside x/y planes.
    dx = max(xs) - min(xs)
    dy = max(ys) - min(ys)
    dz = max(zs) - min(zs)
    ...
    # Standard end perimeter edges live on either extreme longitudinal plane
    # and on one of the outside profile planes.
    at_end = (
        all(abs(abs(z) - half_len) <= tol_z for z in zs)
        or (max(zs) - min(zs) <= tol_z and abs(abs(sum(zs)/len(zs)) - half_len) <= tol_z)
    )
    if at_end:
        on_x = all(abs(abs(x) - hpw) <= tol_xy for x in xs)
        on_y = all(abs(abs(y) - hph) <= tol_xy for y in ys)
    ...
    # Straight transverse edge wholly on an outer face and spanning only the
    # expected profile dimension is still standard tube geometry.
    if gt == "LINE":
        ...
        if on_outer_x or on_outer_y:
            # Intermediate short edges on a face are exactly what slots,
            # notches and tabs introduce, so only call them standard if they
            # are at an end or longitudinal.
            if at_end or dz > max(dx, dy) * 5.0:
                return True

    # Anything else is a likely machining feature.
    return False
...
def _feature_polylines(
    ...
    """
    Return only likely extra-machining edges.
    Plain profiles return an empty list.
    """
    if not detail or not detail.get("recognized"):
        return []
    ...
    try:
        for edge in solid.Edges():
            if _is_standard_profile_edge(edge, solid, detail):
                continue
            pts = _edge_points(edge)
            if len(pts) >= 2:
                features.append(pts)
