...
def _profile_basis_for_features(
    ...
    # v385: learn the profile's own longitudinal seam/tangent positions from
    # the STEP itself. Rounded rectangular tube corners are cylindrical faces;
    # the boundaries of those radius faces are STRAIGHT lines along the tube.
    # Therefore checking only CIRCLE edges can never be reliable.
    #
    # We take only long axis-parallel edges as anchors. Short parallel edges
    # created by a notch/slot are not allowed to teach the profile shape.
    ...
    for edge in solid.Edges():
        ...
        mid = ((p + q) / 2.0) - c
        ax = float(np.dot(mid, xdir))
        ay = float(np.dot(mid, ydir))
        seam_anchors.append((ax, ay))

    # Deduplicate tangent positions. Tolerance scales gently with profile size
    # but stays tight enough that a nearby slot wall is not treated as stock.
    anchor_tol = max(0.35, min(max(ow, 1.0), max(oh, 1.0)) * 0.012)
    ...
    return c, xdir, ydir, zdir, ow, oh, half_len, unique_anchors, anchor_tol
...
def _is_standard_profile_edge(
    ...
    """
    True = purchased profile geometry (do NOT highlight as laser cut).

    Dimension-independent rules:
    1. Learn the real longitudinal seam/tangent positions of the profile from
       long axis-parallel STEP edges. This catches rounded corners even when
       the radius is represented by straight tangent boundaries.
    2. Any later axis-parallel edge at one of those same transverse positions
       is a split piece of the same stock seam/radius boundary, not a cut.
    3. A curved edge (CIRCLE/BSPLINE/ELLIPSE/BEZIER) is profile-radius geometry
       only when BOTH of its ends land on learned stock seam anchors. This is
       what removes oblique radius traces without hiding drilled holes/slots.
    4. Everything else is real cut geometry: complete straight/mitred ends,
       notches, slots, holes, tabs and cut-outs.
    """
    c, xdir, ydir, zdir, ow, oh, half_len, seam_anchors, anchor_tol = basis
    ...
    def near_anchor(x: float, y: float, factor: float = 1.0) -> bool:
    ...
    if gt == "LINE":
        ...
        if length > 1e-7:
            ...
            if axis_ratio >= 0.9985:
                ...
                # Split portions of a learned longitudinal radius/tangent seam.
                if near_anchor(mx, my, 1.35):
                    return True

        # Every non-stock straight edge is an actual cut contour.
        return False

    if gt in {"CIRCLE", "ELLIPSE", "BSPLINE", "BEZIER"}:
        # A profile-radius trace begins and ends exactly where the radius face
        # meets its neighbouring flat faces: at learned longitudinal anchors.
        # Sampled BSPLINE geometry is used here, so slanted end cuts are safe.
        x0, y0, _ = coords[0]
        x1, y1, _ = coords[-1]
        if seam_anchors and near_anchor(x0, y0, 2.0) and near_anchor(x1, y1, 2.0):
            return True

        # Drilled holes, slot arcs and arbitrary curves remain machining.
        return False
