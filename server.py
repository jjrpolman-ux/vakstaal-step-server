...
STEP_PROFILE_RECOGNITION_VERSION = 10  # robust main-wall / coaxial-cylinder recognition
...
def _lcm_value_table_and_laser_field_map(data: bytes) -> tuple[dict[int,str],dict[int,str]]:
    ...
    return table,{0x13+i:name for i,name in enumerate(ordered_fields)}
...
def _lcm_cut_parameters_complete(data: bytes,cut_block: dict) -> dict:
    ...
    return {
        ...
        "cutHeightMm":cut_block.get("Height"),
        "peakPowerPct":cut_block.get("LaserCurrent"),
        "dutyCyclePct":(float(ratio)*100.0 if isinstance(ratio,(int,float)) and abs(float(ratio))<=1.5 else ratio),
        "frequencyHz":cut_block.get("PwmFreq"),
        "beamSizeMm":cut_block.get("BeamSize"),
        "pierceTimeMs":cut_block.get("CycleTime"),
...
def _lcm_corner_parameters_complete(data: bytes) -> dict:
    ...
    return {
        ...
        "bAxisAcceleration":_lcm_read_named_number(data,b"PTConsA"),
...
def _parse_fs_material_lcm(content: bytes, filename: str = "") -> dict:
    ...
    speed_m_min = (
        ...
        if work_speed_mm_s is not None else None
    )
    ...
    return {
        ...
        "focusMm":focus,"cutGasMachine":_lcm_gas_name(cut_values.get("GasType")),
