...
STEP_MATERIAL_LENGTH_VERSION = 2  # longest physical extent along real profile axis
...
def _physical_material_length_along_axis(
    ...
    """
    Material-/nestinglengte voor een STEP-body.

    Definitie:
    - bepaal eerst de echte longitudinale profielas;
    - projecteer ALLE echte geometrische hoekpunten op die as;
    - neem max(projectie) - min(projectie).

    Daardoor wordt bij rechte, schuine of onderling verschillende eindhoeken
    altijd de LANGSTE fysiek benodigde lengte gebruikt. De oriëntatie van de
    body in het STEP-assenstelsel heeft geen invloed.

    Deze lengte is uitsluitend materiaal/nestinglengte. Snijlengte komt uit de
    daadwerkelijke fysieke snijcontouren en blijft volledig gescheiden.
    """
    ...
    # Vertices zijn exacte topologische hoekpunten en omvatten ook schuine,
    # verstek-, tab- en andere uiteinden.
    for vertex in solid.Vertices():
        try:
            p = np.array(vertex.toTuple(), dtype=float)
            projected.append(float(np.dot(p, axis)))
        except Exception:
            continue

    # Extra zekerheid voor geometrie waarin extrema niet als los Vertex-object
    # worden teruggegeven: neem ook edge-eindpunten mee.
    ...
    if len(projected) < 2:
        # Alleen als geometrische projectie onmogelijk is terugvallen op analyzer.
        return max(0.0, float(analyzer_length or 0.0)), axis, f"{method}-fallback"

    physical_length = max(projected) - min(projected)
    if not math.isfinite(physical_length) or physical_length <= 0:
        physical_length = max(0.0, float(analyzer_length or 0.0))
        return physical_length, axis, f"{method}-fallback"

    return float(physical_length), axis, "physical-longest-axis-projection-v2"
...
def _apply_physical_material_lengths(step_path: Path, result: dict) -> dict:
    ...
    result["material_length_definition"] = (
        "longest physical body extent projected on the real longitudinal profile axis"
    )
