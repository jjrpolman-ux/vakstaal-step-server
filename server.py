...
from fastapi import FastAPI, File, Form, HTTPException, UploadFile, Response, Body
...
# v55: volledige uitsparingsdetectie + expliciete onbekende profielmetadata
...
def _assembly_cache_path(job_id: str) -> Path:
    return CACHE_DIR / job_id / "assembly_mesh_detail_v55.json"
...
def _is_standard_profile_edge(
    ...
    # v52: curved geometry is NOT automatically a machining feature.
    # Normal rounded tube corners also generate circles/splines in STEP files.
    # Classify them geometrically first; only curves away from the normal
    # profile boundary are treated as holes/slots/cut-outs.
    ...
    # Standard longitudinal profile edges. Besides the flat-side edges,
    # rounded RHS/SHS corners create long straight tangent/seam edges which
    # sit slightly inside BOTH nominal outer dimensions. Those are normal
    # profile geometry too and must never become machining/snijlijnen.
    member_len = max(1.0, half_len * 2.0)
    if dz > max(dx, dy) * 5.0 and dz >= member_len * 0.90:
        on_x = all(abs(abs(x) - hpw) <= tol_xy for x in xs)
        on_y = all(abs(abs(y) - hph) <= tol_xy for y in ys)
        if on_x or on_y:
            return True

        # Radius-aware corner zone. Scale with profile size instead of using a
        # fixed R8 ceiling: 40x20 stays tight, while e.g. 100x100 with a much
        # larger production radius is still recognized as ordinary profile.
        corner_band = max(2.5, min(30.0, min(ow, oh) * 0.30))
        near_corner = all(
            (hpw - abs(x)) >= -tol_xy
            and (hph - abs(y)) >= -tol_xy
            and (hpw - abs(x)) <= corner_band
            and (hph - abs(y)) <= corner_band
    ...
    if at_end:
        ...
        # Rounded rectangular tube corners at an end plane. Use a real
        # corner-zone test instead of a fixed radius ceiling. This supports
        # both small 40x20 radii and substantially larger radii on 100x100.
        corner_band = max(2.5, min(30.0, min(ow, oh) * 0.30))
        near_corner = all(
            (hpw - abs(x)) >= -tol_xy
            and (hph - abs(y)) >= -tol_xy
    ...
    if gt == "LINE":
        ...
        if (on_outer_x or on_outer_y) and (
            at_end or (dz > max(dx, dy) * 5.0 and dz >= member_len * 0.90)
        ):
            return True
...
def _base_end_cut_polylines(
    solid: cq.Shape,
    detail: dict | None,
) -> list[list[list[float]]]:
    """Exacte buitencontour van één profieluiteinde rechtstreeks uit STEP/BREP.

    Deze lijnen zijn de basis-afkortcontour. Ze worden APART van feature_lines
    teruggegeven, zodat de viewer echte radii kan tonen zonder de snijlengte
    dubbel te tellen.

    Belangrijk: gebruik topologische STEP-edges, niet de getesselleerde mesh.
    Daardoor blijven afgeronde RHS/SHS-hoeken als echte boogsegmenten aanwezig.
    """
    if not detail or not detail.get("recognized"):
        return []
    ...
    c, xdir, ydir, zdir, ow, oh, half_len = basis
    if ow <= 0 or oh <= 0 or half_len <= 0:
        return []

    profile_type = str(detail.get("type") or "").lower()
    round_profile = "rond" in profile_type

    # Kies consequent het positieve profieluiteinde.
    target_z = float(half_len)
    member_len = max(1.0, 2.0 * float(half_len))
    tol_z = max(0.35, member_len * 0.0015)

    hpw = max(0.001, float(ow) / 2.0)
    hph = max(0.001, float(oh) / 2.0)
    ...
    for edge in solid.Edges():
        ...
        # De hele edge moet praktisch in hetzelfde eindvlak liggen.
        if max(zs) - min(zs) > tol_z * 2.0:
            continue

        mean_z = sum(zs) / len(zs)
        if abs(mean_z - target_z) > tol_z * 3.0:
            continue

        # Buiten- en binnencontour zijn beide topologische randen aan een
        # open buiseinde. Selecteer uitsluitend de buitenste contour.
        if round_profile:
            radial = [
                math.sqrt((x / hpw) ** 2 + (y / hph) ** 2)
                for x, y, _z in coords
            ]
            outer_score = sum(radial) / len(radial)
        else:
            # Voor rechthoek/RHS: een buitenrand raakt de nominale buitenmaat.
            # max(|x|/hpw, |y|/hph) is ~1 voor buitencontour, duidelijk kleiner
            # voor de binnencontour. Ook afgeronde buitenhoeken blijven ~1.
            radial = [
                max(abs(x) / hpw, abs(y) / hph)
                for x, y, _z in coords
            ]
            outer_score = sum(radial) / len(radial)

        candidates.append((outer_score, pts))

    if not candidates:
        return []

    # Bepaal de buitenste score dynamisch. Dit werkt ook bij zeer dunwandige
    # profielen waar de binnencontour dicht bij de buitencontour ligt.
    max_score = max(score for score, _pts in candidates)
    threshold = max(0.90, max_score - 0.018)

    selected = [
        pts for score, pts in candidates
        if score >= threshold
    ]
    ...
    # Topologische dubbelingen verwijderen.
    tol = 0.05

    def point_key(p):
        return tuple(int(round(float(v) / tol)) for v in p[:3])
    ...
    for pts in selected:
        ...
        b = point_key(pts[-1])
        lo, hi = sorted((a, b))
        length_key = int(round(_polyline_length_mm(pts) / tol))
        key = (lo, hi, length_key, len(pts))
        if key in seen:
            continue
        seen.add(key)
        result.append(pts)
...
def _feature_polylines(
    ...
    """Return all non-standard profile edges that can represent laser machining.

    v59 deliberately avoids the former 'closed component only' rule. That rule
    removed valid vertical edges of open notches/hapjes when a STEP exporter
    split the contour into separate BREP edges. Standard tube geometry is still
    removed by _is_standard_profile_edge(); every remaining credible edge is
    returned so the 3D viewer and cut-length calculation use the same complete set.
    """
    if not detail or not detail.get("recognized"):
        return []
    ...
    try:
        member_len = max(1.0, float(basis[6]) * 2.0)
        axis = np.array(basis[3], dtype=float)
        ...
        for edge in solid.Edges():
            ...
            # Exclude only a near-full-length longitudinal seam/split.
            # Short axial legs belonging to notches/slots must stay visible.
            arr = np.array(pts, dtype=float)
            proj = arr @ axis
            axial_span = float(proj.max() - proj.min()) if len(proj) else 0.0
            if axial_span >= max(150.0, member_len * 0.965):
                continue

            if _polyline_length_mm(pts) < 0.05:
                continue
    ...
    # Deduplicate BREP duplicates, but do NOT require contours to be closed.
    # Endpoints are direction-independent. Length is included to distinguish
    # separate curved edges that happen to share almost equal endpoints.
    tol = 0.08
...
def _polyline_length_mm(poly: list[list[float]]) -> float:
    total = 0.0
...
def _machining_length_mm(feature_lines: list[list[list[float]]]) -> float:
    return float(sum(_polyline_length_mm(poly) for poly in (feature_lines or [])))
...
VAKSTAAL_SYSTEM_DROPBOX_FOLDER = f"{DROPBOX_ROOT.rstrip('/')}/_Vakstaal"
VAKSTAAL_APP_STATE_DROPBOX_PATH = f"{VAKSTAAL_SYSTEM_DROPBOX_FOLDER}/app_state.json"


def _dropbox_list_recursive(root: str) -> list[dict]:
    """Lijst Dropbox recursief; gebruikt paging zodat ook oudere offertemappen gevonden worden."""
    result = _dropbox_rpc(
        "files/list_folder",
        {
            "path": root,
...
def _load_app_state_from_dropbox() -> tuple[dict | None, str | None]:
    try:
        raw = _dropbox_download_bytes(VAKSTAAL_APP_STATE_DROPBOX_PATH)
        wrapper = json.loads(raw.decode("utf-8"))
        state = wrapper.get("state") if isinstance(wrapper, dict) else None
        updated_at = wrapper.get("updated_at") if isinstance(wrapper, dict) else None
...
def _backup_app_state_to_dropbox(state: dict, updated_at: str) -> None:
    try:
        wrapper = {
        ...
        _dropbox_upload_bytes(
            VAKSTAAL_APP_STATE_DROPBOX_PATH,
            json.dumps(wrapper, ensure_ascii=False, indent=2).encode("utf-8"),
        )
    except Exception:
        # Database blijft de primaire bron; Dropbox-backup mag opslaan nooit blokkeren.
        pass


def _insert_recovered_quote(conn, wrapper: dict) -> bool:
    payload = wrapper.get("payload") if isinstance(wrapper, dict) else None
    if not isinstance(payload, dict):
        return False

    quote_number = str(
        wrapper.get("quote_number")
    ...
    quote_id = str(wrapper.get("quote_id") or uuid.uuid4().hex).strip()
    customer_name = str(
        wrapper.get("customer_name")
        or payload.get("customer")
        or "Klant"
    ).strip() or "Klant"

    contact = str(payload.get("contactPerson") or "")
    email = str(payload.get("customerEmail") or "")
    phone = str(payload.get("customerPhone") or "")
    total = float(payload.get("total_ex_vat") or 0)
    stamp = str(wrapper.get("saved_at") or payload.get("saved_at") or _utcnow())
    ...
    if _postgres_enabled():
        cur.execute(
            ...
            """,
            (quote_id, quote_number, customer_name, contact, email, phone, total, raw, stamp, stamp),
        )
    else:
        cur.execute(
            """
...
def _recover_quotes_from_dropbox(conn) -> int:
    """Herstel offerte-metadata uit bestaande offerte.json-bestanden wanneer de DB leeg is."""
    try:
        entries = _dropbox_list_recursive(DROPBOX_ROOT)
    except Exception:
        return 0

    recovered = 0
    for entry in entries:
        if str(entry.get(".tag") or "") != "file":
            continue
        if str(entry.get("name") or "").lower() != "offerte.json":
            continue
        path = entry.get("path_display") or entry.get("path_lower")
        if not path:
            continue
        try:
            wrapper = json.loads(_dropbox_download_bytes(path).decode("utf-8"))
            if _insert_recovered_quote(conn, wrapper):
                recovered += 1
        except Exception:
            continue
...
def _storage_config_from_payload(payload_json: str) -> dict:
    """Gebruik zonder expliciete instellingen exact de historische v55-mapstructuur."""
    defaults = {
        "dropboxRoot": "Offertes",
        "quotesFolder": "",
        "useYearFolder": True,
        "quoteFolderPattern": "{offertenummer} - {klant}",
        "stepFolder": "Productie STEP",
        "pdfFolder": "PDF",
        "sourceFolder": "Origineel",
        "nestFolder": "Nest",
        "otherFolder": "Overige bestanden",
    }
    try:
...
def _safe_dropbox_relative_path(value: str) -> str:
    parts = [p for p in re.split(r"[\\/]+", str(value or "")) if p.strip()]
    return "/".join(_safe_dropbox_name(p, "Map") for p in parts)

def _quote_dropbox_folder(quote_number: str, customer_name: str, payload_json: str = "") -> str:
    cfg = _storage_config_from_payload(payload_json)
    configured = _safe_dropbox_relative_path(cfg.get("quotePath", ""))
    if configured:
...
def _configured_subfolder(payload_json: str, filename: str) -> str:
    cfg = _storage_config_from_payload(payload_json)
    ...
    if suffix == ".pdf":
        ...
        is_generated = bool(re.match(r"^VAK-20\d{2}-", str(filename or ""), re.I)) or any(w in lower for w in production_words)
        key = "stepFolder" if is_generated else "sourceFolder"
    elif suffix in {".zx", ".nest"}:
        key = "nestFolder"
    else:
        key = "otherFolder"
    chosen = _safe_dropbox_relative_path(cfg.get(key, ""))
    return chosen or _file_dropbox_subfolder(filename)
...
def _sync_quote_json_to_dropbox(conn, quote_id: str) -> str:
    ...
    folder = _quote_dropbox_folder(quote_number, customer_name, payload_json)
    try:
...
async def _store_quote_files(conn, quote_id: str, files: list[UploadFile]) -> None:
    ...
    folder = _quote_dropbox_folder(quote_number, customer_name, _payload_json)
    ...
    for upload in files or []:
        ...
        subfolder = _configured_subfolder(_payload_json, filename)
        ...
        if existing:
            ...
            cur.execute(
                ...
                (
                    ...
                    len(data),
                    actual_path,
                    data,  # altijd ook in database bewaren als veilige fallback
                    _utcnow(),
                    existing_id,
                )
            )
...
def get_app_state():
    with _db_connect() as conn:
        ...
        if not row:
            # Database kan na een deploy/fallback leeg lijken. De centraal
            # opgeslagen standaardinstellingen worden daarom ook uit Dropbox hersteld.
            dropbox_state, dropbox_updated_at = _load_app_state_from_dropbox()
            if isinstance(dropbox_state, dict):
                now = dropbox_updated_at or _utcnow()
                raw = json.dumps(dropbox_state, ensure_ascii=False)
                if _postgres_enabled():
                    cur.execute(
                        """
...
def put_app_state(payload: dict):
    ...
    _backup_app_state_to_dropbox(state, now)
...
def list_quotes():
    with _db_connect() as conn:
        ...
        first = cur.fetchone()
        count = int(first[0] if not isinstance(first, sqlite3.Row) else list(first)[0])

        recovered = 0
        if count == 0:
            recovered = _recover_quotes_from_dropbox(conn)
        ...
        return {
            ...
            "quotes": rows,
            "recovered_from_dropbox": recovered,
        }
...
def delete_quote(quote_id: str):
    with _db_connect() as conn:
        ...
        if isinstance(row, sqlite3.Row):
            ...
            quote_number, customer_name, payload_json = row

        dropbox_folder = _quote_dropbox_folder(quote_number, customer_name, payload_json)
...
def database_status():
    with _db_connect() as conn:
        ...
        cur.execute("SELECT COUNT(*) FROM quotes")
        qrow = cur.fetchone()
        quote_count = int(qrow[0] if not isinstance(qrow, sqlite3.Row) else list(qrow)[0])
        cur.execute("SELECT COUNT(*) FROM app_state")
        arow = cur.fetchone()
        state_count = int(arow[0] if not isinstance(arow, sqlite3.Row) else list(arow)[0])
    return {
        "ok": True,
        "database": "postgresql" if _postgres_enabled() else "sqlite",
        "database_url_configured": bool(DATABASE_URL),
...
# ============================================================================
# e-Boekhouden REST koppeling (server-side token; nooit naar browser sturen)
# ============================================================================
EBOEKHOUDEN_API_TOKEN = os.environ.get("EBOEKHOUDEN_API_TOKEN", "").strip()
EBOEKHOUDEN_API_BASE = os.environ.get("EBOEKHOUDEN_API_BASE", "https://api.e-boekhouden.nl").rstrip("/")
EBOEKHOUDEN_SOURCE = (os.environ.get("EBOEKHOUDEN_SOURCE", "Vakstaal") or "Vakstaal")[:10]
EBOEKHOUDEN_LEDGER_CODE = os.environ.get("EBOEKHOUDEN_LEDGER_CODE", "8055").strip()
EBOEKHOUDEN_TEMPLATE_NAME = os.environ.get("EBOEKHOUDEN_TEMPLATE_NAME", "Vakstaal").strip()
EBOEKHOUDEN_TERM_DAYS = int(os.environ.get("EBOEKHOUDEN_TERM_DAYS", "30"))
EBOEKHOUDEN_INVOICE_ENDPOINT = os.environ.get("EBOEKHOUDEN_INVOICE_ENDPOINT", "/v1/invoice").strip() or "/v1/invoice"
EBOEKHOUDEN_RELATION_ENDPOINT = os.environ.get("EBOEKHOUDEN_RELATION_ENDPOINT", "/v1/relation").strip() or "/v1/relation"
EBOEKHOUDEN_LEDGER_ENDPOINT = os.environ.get("EBOEKHOUDEN_LEDGER_ENDPOINT", "/v1/ledger").strip() or "/v1/ledger"
EBOEKHOUDEN_TEMPLATE_ENDPOINT = os.environ.get("EBOEKHOUDEN_TEMPLATE_ENDPOINT", "/v1/invoice-template").strip() or "/v1/invoice-template"
_eboek_session_token = ""
_eboek_session_expires = 0.0


def _eboek_json_request(method: str, path: str, body: dict | None = None, query: dict | None = None, auth: bool = True):
    global _eboek_session_token, _eboek_session_expires
    if not EBOEKHOUDEN_API_TOKEN:
        raise HTTPException(status_code=503, detail="EBOEKHOUDEN_API_TOKEN is nog niet ingesteld op de server.")
    if auth and (not _eboek_session_token or time.time() >= _eboek_session_expires):
        req = urllib.request.Request(
            EBOEKHOUDEN_API_BASE + "/v1/session",
            data=json.dumps({"accessToken": EBOEKHOUDEN_API_TOKEN, "source": EBOEKHOUDEN_SOURCE}).encode("utf-8"),
            method="POST",
            headers={"Content-Type": "application/json"},
        )
        try:
            with urllib.request.urlopen(req, timeout=30) as response:
                sess = json.loads(response.read().decode("utf-8"))
        except urllib.error.HTTPError as exc:
            detail = exc.read().decode("utf-8", errors="replace")
            raise HTTPException(status_code=502, detail=f"e-Boekhouden sessie mislukt: {detail[:700]}") from exc
        token = str(sess.get("token") or sess.get("sessionToken") or sess.get("accessToken") or "").strip()
        if not token:
            raise HTTPException(status_code=502, detail="e-Boekhouden gaf geen sessietoken terug.")
        _eboek_session_token = token
        _eboek_session_expires = time.time() + 3300

    url = EBOEKHOUDEN_API_BASE + path
    if query:
        url += "?" + urllib.parse.urlencode({k:v for k,v in query.items() if v is not None})
    headers={"Content-Type":"application/json"}
    if auth:
        # Officiële quick-start gebruikt het sessietoken direct als Authorization-waarde.
        headers["Authorization"]=_eboek_session_token
    req=urllib.request.Request(url,data=(json.dumps(body).encode("utf-8") if body is not None else None),method=method,headers=headers)
    try:
        ...
        if exc.code==401 and auth:
            _eboek_session_token=""
            _eboek_session_expires=0
        raise HTTPException(status_code=502,detail=f"e-Boekhouden API fout ({exc.code}): {detail[:1200]}") from exc


def _eboek_items(value):
    """Haal een resultatenlijst uit de verschillende response-wrappers van de REST API."""
    if isinstance(value, list):
        return value
    if isinstance(value, dict):
        for key in ("items", "data", "results", "relations", "ledgers", "templates", "invoices"):
            child = value.get(key)
            if isinstance(child, list):
                return child
            if isinstance(child, dict):
                nested = _eboek_items(child)
                if nested:
                    return nested
    return []


def _eboek_get_first_working(paths: list[str], *, query: dict | None = None):
    """Probeer route-aliassen zonder dat de eerste fout de hele lookup afbreekt."""
    errors = []
    for path in paths:
        if not path:
            continue
        try:
            return _eboek_json_request("GET", path, query=query)
        except HTTPException as exc:
            errors.append(f"{path}: {exc.detail}")
    if errors:
        raise HTTPException(
            status_code=502,
            detail=("e-Boekhouden lookup mislukt. " + " | ".join(errors))[:1800],
        )
    return {}


def _eboek_first_id(value):
    if isinstance(value,dict):
        for key in ("id","relationId","invoiceId","ledgerId","templateId"):
            if value.get(key) is not None: return value.get(key)
    return None


def _eboek_find_relation(payload: dict):
    email=str(payload.get("customerEmail") or "").strip()
    name=str(payload.get("customer") or "").strip()
    queries=[]
    if email: queries.append({"emailAddress":email,"limit":50})
    if name: queries.append({"name":name,"limit":50})
    for q in queries:
        rows=_eboek_items(_eboek_json_request("GET",EBOEKHOUDEN_RELATION_ENDPOINT,query=q))
        if rows:
            if email:
                exact=next((r for r in rows if str(r.get("emailAddress") or r.get("email") or "").lower()==email.lower()),None)
                if exact: return exact
            exact=next((r for r in rows if str(r.get("name") or "").strip().lower()==name.lower()),None)
            return exact or rows[0]
    return None


def _eboek_create_relation(payload: dict):
    name=str(payload.get("customer") or "").strip()
    if not name:
        raise HTTPException(status_code=400,detail="Klantnaam ontbreekt.")
    body={
        "type":"B",
        "name":name,
        "contact":str(payload.get("contactPerson") or "")[:100],
        "address":str(payload.get("customerAddress") or "")[:100],
        "phoneNumber":str(payload.get("customerPhone") or "")[:50],
        "emailAddress":str(payload.get("customerEmail") or "")[:100],
        "termOfPayment":EBOEKHOUDEN_TERM_DAYS,
    }
    return _eboek_json_request("POST",EBOEKHOUDEN_RELATION_ENDPOINT,body=body)


def _eboek_find_ledger_id():
    rows=_eboek_items(_eboek_json_request("GET",EBOEKHOUDEN_LEDGER_ENDPOINT,query={"code":EBOEKHOUDEN_LEDGER_CODE,"limit":50}))
    row=next((r for r in rows if str(r.get("code") or "").strip()==EBOEKHOUDEN_LEDGER_CODE),None)
    if not row:
        raise HTTPException(status_code=422,detail=f"Grootboek {EBOEKHOUDEN_LEDGER_CODE} niet gevonden in e-Boekhouden.")
    return _eboek_first_id(row)


def _eboek_find_template_id():
    routes = list(dict.fromkeys([
        EBOEKHOUDEN_TEMPLATE_ENDPOINT,
        "/v1/invoiceTemplate",
        "/v1/invoice-template",
    ]))
    result = _eboek_get_first_working(
        routes,
        query={"name": EBOEKHOUDEN_TEMPLATE_NAME, "limit": 100},
    )
    rows = _eboek_items(result)
    row = next(
        (
            r for r in rows
            if str(r.get("name") or r.get("description") or "").strip().lower()
            == EBOEKHOUDEN_TEMPLATE_NAME.lower()
        ),
        None,
    ...
    if not row:
        raise HTTPException(
            status_code=422,
            detail=f"Factuursjabloon '{EBOEKHOUDEN_TEMPLATE_NAME}' niet gevonden in e-Boekhouden.",
        )
    template_id = _eboek_first_id(row)
    if template_id is None:
        raise HTTPException(
            status_code=502,
            detail=f"Factuursjabloon '{EBOEKHOUDEN_TEMPLATE_NAME}' gevonden, maar zonder bruikbaar ID.",
        )
    return template_id
...
def eboekhouden_status():
    return {
        "ok":True,
        "configured":bool(EBOEKHOUDEN_API_TOKEN),
        "ledger_code":EBOEKHOUDEN_LEDGER_CODE,
        "template":EBOEKHOUDEN_TEMPLATE_NAME,
        "term_days":EBOEKHOUDEN_TERM_DAYS,
        "invoice_endpoint":EBOEKHOUDEN_INVOICE_ENDPOINT,
        "template_endpoint":EBOEKHOUDEN_TEMPLATE_ENDPOINT,
    }
...
def create_eboekhouden_invoice(quote_id: str):
    with _db_connect() as conn:
        cur=conn.cursor()
        cur.execute(_sql("SELECT quote_number, customer_name, total_ex_vat, payload_json FROM quotes WHERE id=%s","SELECT quote_number, customer_name, total_ex_vat, payload_json FROM quotes WHERE id=?"),(quote_id,))
        row=cur.fetchone()
        if not row: raise HTTPException(status_code=404,detail="Offerte niet gevonden.")
        if isinstance(row,sqlite3.Row):
            quote_number=row["quote_number"]; total_ex_vat=float(row["total_ex_vat"] or 0); payload_json=row["payload_json"]
        else:
            quote_number, _customer_name, total_ex_vat, payload_json=row; total_ex_vat=float(total_ex_vat or 0)
        try: payload=json.loads(payload_json or "{}")
        except Exception: payload={}
        existing=payload.get("eboekhouden_invoice")
        if isinstance(existing,dict) and existing.get("id"):
            return {"ok":True,"already_created":True,"invoice":existing}

        relation=_eboek_find_relation(payload)
        if relation is None:
            relation=_eboek_create_relation(payload)
        relation_id=_eboek_first_id(relation)
        if relation_id is None:
            raise HTTPException(status_code=502,detail="e-Boekhouden relatie-ID ontbreekt in antwoord.")

        ledger_id=_eboek_find_ledger_id()
        template_id=_eboek_find_template_id()
        description=f"Werkzaamheden volgens offerte {quote_number}"
        order_number=str(payload.get("customerOrderNumber") or "").strip()
        body={
            "relationId":relation_id,
            "date":datetime.now().date().isoformat(),
            "termOfPayment":EBOEKHOUDEN_TERM_DAYS,
            "templateId":template_id,
            "reference":order_number or quote_number,
            "inExVat":"EX",
            "items":[{
                "description":description,
                "pricePerUnit":round(total_ex_vat,2),
                "quantity":1,
                "vatCode":"HOOG_VERK_21",
                "ledgerId":ledger_id,
            }],
        }
        invoice=_eboek_json_request("POST",EBOEKHOUDEN_INVOICE_ENDPOINT,body=body)
        invoice_id=_eboek_first_id(invoice)
        marker={
            "id":invoice_id,
            "invoice_number":(
        ...
        payload["eboekhouden_invoice"]=marker
        cur.execute(_sql("UPDATE quotes SET payload_json=%s, updated_at=%s WHERE id=%s","UPDATE quotes SET payload_json=?, updated_at=? WHERE id=?"),(json.dumps(payload,ensure_ascii=False),_utcnow(),quote_id))
        conn.commit()
        try: _sync_quote_json_to_dropbox(conn,quote_id)
        except Exception: pass
        return {"ok":True,"already_created":False,"invoice":marker}
...
# ============================================================================
# PRODUCTIE-STEP GENERATIE
# Browser stuurt alleen profielgegevens + losse lengtes.
# CadQuery bouwt één STEP-bestand met alle kokers als afzonderlijke solids.
# ============================================================================

def _production_step_tube(
    width: float,
    height: float,
    wall: float,
    radius: float,
    ...
    if width <= 0 or height <= 0 or wall <= 0 or length <= 0:
        raise ValueError("Ongeldige kokerafmetingen.")
    if width - 2 * wall <= 0 or height - 2 * wall <= 0:
        raise ValueError("Wanddikte is te groot voor deze koker.")

    max_outer_radius = max(0.01, min(width, height) / 2.0 - 0.01)
    outer_radius = min(max(0.01, radius), max_outer_radius)

    inner_width = width - 2 * wall
    inner_height = height - 2 * wall
    inner_radius = max(0.01, outer_radius - wall)
    inner_radius = min(
        inner_radius,
        max(0.01, min(inner_width, inner_height) / 2.0 - 0.01),
    )
...
def generate_production_step(payload: dict):
    try:
        profile_size = str(payload.get("size") or "Koker").strip()
        width = float(payload.get("outerWidth") or 0)
        height = float(payload.get("outerHeight") or 0)
        wall = float(payload.get("wallThickness") or 0)
        radius = float(payload.get("radius") or 0)
        pieces = payload.get("pieces") or []

        if not isinstance(pieces, list) or not pieces:
            raise ValueError("Geen kokers opgegeven.")
        if len(pieces) > 1000:
            raise ValueError("Te veel kokers in één productie-STEP.")

        lengths = []
        for item in pieces:
            value = item.get("lengthMm") if isinstance(item, dict) else item
            length = float(value or 0)
            if length <= 0:
                raise ValueError("Ongeldige kokerlengte.")
            lengths.append(length)

        spacing = max(width, height) + 25.0
        cols = max(1, int(math.ceil(math.sqrt(len(lengths)))))

        solids = []
        for index, length in enumerate(lengths):
            wp = _production_step_tube(width, height, wall, radius, length)
            col = index % cols
            row = index // cols
            wp = wp.translate((col * spacing, row * spacing, 0.0))
            solids.append(wp.val())

        compound = cq.Compound.makeCompound(solids)

        folder = CACHE_DIR / ("production_" + uuid.uuid4().hex)
        folder.mkdir(parents=True, exist_ok=True)
        step_path = folder / "productie.step"

        cq.exporters.export(compound, str(step_path))
        data = step_path.read_bytes()

        if not data:
            raise ValueError("CadQuery heeft een leeg STEP-bestand aangemaakt.")

        safe_size = _safe_dropbox_name(profile_size, "Koker")
        filename = f"productie_{safe_size}_{len(lengths)}st.step"
        ...
        return Response(
            ...
            headers={
                ...
                "X-Vakstaal-Solid-Count": str(len(lengths)),
                "X-Content-Type-Options": "nosniff",
            },
        )
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Productie-STEP kon niet worden gemaakt: {type(exc).__name__}: {exc}",
        ) from exc
...
def root():
    return {
        ...
        "endpoints": [
            ...
            "/api/quotes/{quote_id}/eboekhouden-invoice",
        ],
    }



def _export_selected_step_bytes(step_path: Path, solid_indices: list[int]) -> bytes:
    imported = cq.importers.importStep(str(step_path))
    solids = imported.solids().vals()
    wanted = []
    seen = set()
    for raw in solid_indices or []:
        try:
            idx = int(raw)
        except Exception:
            continue
        if idx in seen or idx < 1 or idx > len(solids):
            continue
        seen.add(idx)
        wanted.append(solids[idx - 1])
    if not wanted:
        raise HTTPException(status_code=400, detail="Geen STEP-bodies geselecteerd.")

    temp_dir = CACHE_DIR / ("filtered_" + uuid.uuid4().hex)
    temp_dir.mkdir(parents=True, exist_ok=True)
    out_path = temp_dir / "filtered.step"
    try:
        shape = wanted[0] if len(wanted) == 1 else cq.Compound.makeCompound(wanted)
        cq.exporters.export(shape, str(out_path))
        data = out_path.read_bytes()
        if not data:
            raise ValueError("Leeg STEP-resultaat.")
        return data
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=422, detail=f"Gefilterde STEP kon niet worden gemaakt: {type(exc).__name__}: {exc}") from exc
    finally:
        shutil.rmtree(temp_dir, ignore_errors=True)


def _quote_file_bytes_by_name(conn, quote_id: str, filename: str) -> bytes:
    cur = conn.cursor()
    cur.execute(
        _sql(
            """SELECT data, dropbox_path FROM quote_files WHERE quote_id=%s AND filename=%s ORDER BY created_at DESC LIMIT 1""",
            """SELECT data, dropbox_path FROM quote_files WHERE quote_id=? AND filename=? ORDER BY created_at DESC LIMIT 1""",
        ),
        (quote_id, filename),
    )
    row = cur.fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Origineel STEP-bestand niet gevonden bij deze offerte.")
    if isinstance(row, sqlite3.Row):
        data, dropbox_path = row["data"], row["dropbox_path"]
    else:
        data, dropbox_path = row
    if dropbox_path:
        try:
            return _dropbox_download_bytes(dropbox_path)
        except HTTPException:
            pass
    if data:
        return bytes(data)
    raise HTTPException(status_code=404, detail="Originele STEP-inhoud is niet beschikbaar.")


@app.post("/api/filter-step/{job_id}")
def filter_step_job(job_id: str, body: dict = Body(...)):
    cleanup_old_jobs()
    indices = body.get("solid_indices") or []
    data = _export_selected_step_bytes(job_step_path(job_id), indices)
    return Response(
        content=data,
        media_type="application/step",
        headers={"Content-Disposition": 'attachment; filename="filtered.step"'},
    )


@app.post("/api/quotes/{quote_id}/filtered-step")
def filter_step_quote(quote_id: str, body: dict = Body(...)):
    filename = str(body.get("source_filename") or "").strip()
    indices = body.get("solid_indices") or []
    if not filename:
        raise HTTPException(status_code=400, detail="Bronbestandsnaam ontbreekt.")
    with _db_connect() as conn:
        raw = _quote_file_bytes_by_name(conn, quote_id, filename)
    temp_dir = CACHE_DIR / ("quote_filter_" + uuid.uuid4().hex)
    temp_dir.mkdir(parents=True, exist_ok=True)
    step_path = temp_dir / ("source" + (Path(filename).suffix or ".step"))
    try:
        step_path.write_bytes(raw)
        data = _export_selected_step_bytes(step_path, indices)
...
def assembly_mesh(job_id: str):
    ...
    try:
        ...
        for solid_index, solid in enumerate(solids, start=1):
            ...
            base_cut_lines = _base_end_cut_polylines(solid, detail)

            meshes.append({
                "solid_index": solid_index,
                "recognized": bool((detail or {}).get("recognized")),
                "profile_type": str((detail or {}).get("type") or "Onbekend"),
                "profile_size": str((detail or {}).get("profile_size") or (detail or {}).get("size") or "Onbekend"),
                "vertices": mesh["vertices"],
                "triangles": mesh["triangles"],
                "feature_lines": feature_lines,
                "base_cut_lines": base_cut_lines,
                "has_exact_base_cut": bool(base_cut_lines),
                "has_extra_features": bool(feature_lines),
                "feature_classifier_version": 7,
                "machining_length_mm": _machining_length_mm(feature_lines),
...
def solid_mesh(job_id: str, solid_index: int):
    ...
    try:
        ...
        raw_base_cut_lines = _base_end_cut_polylines(solid, detail)
        ...
        return {
            ...
            "has_extra_features": bool(feature_lines),
            "feature_classifier_version": 7,
            "machining_length_mm": _machining_length_mm(raw_feature_lines),
            "size": mesh["size"],
            "net_geometry": True,
            "trimmed_visual": bool(was_trimmed),
            "feature_preserving_trim": False,
