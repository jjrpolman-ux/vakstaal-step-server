...
async def generate_production_step(request: Request):
    ...
    - ronde holle buizen
    - ronde gaten per product, door één wand of beide tegenoverliggende wanden
    ...
    for piece in pieces:
        ...
        for raw_op in (piece.get("operations") or []):
            raw_op = raw_op or {}
            if str(raw_op.get("type") or "").lower() != "hole":
                continue
            try:
            ...
            if diameter_mm > 0 and 0 < position_mm < length:
                operations.append({
                    "type": "hole",
    ...
    try:
        ...
        def apply_profile_operations(tube, length: float, operations: list):
            # Profielas = Z. Gaten worden haaks op de gekozen wand uitgesneden.
            if not operations:
                return tube
            ...
            for op in operations:
                if str(op.get("type") or "").lower() != "hole":
                    continue

                d = float(op.get("diameterMm") or 0)
                ...
                if face in {"top", "bottom"}:
                    ...
                    if face == "top":
                        start = cq.Vector(offset, cross_h / 2.0 + extra, pos)
                    ...
                    if face == "right":
                        start = cq.Vector(cross_w / 2.0 + extra, offset, pos)
