...
def _is_standard_profile_edge(
    ...
    # Radiusnaden langs de LENGTE zijn profielgeometrie. Dezelfde radius op
    # een fysiek uiteinde hoort juist bij de echte eind-snede en moet zichtbaar
    # blijven. Dit werkt onafhankelijk van de profielmaat.
    if gt == "CIRCLE":
        try:
            r = float(edge.radius())
            center = np.array(edge.arcCenter().toTuple(), dtype=float) - c
            cz = float(np.dot(center, zdir))

            if abs(abs(cz) - half_len) <= max(tol_z * 2.0, 0.8):
                return False
    ...
    if at_end:
        # Het complete fysieke uiteinde is een snijcontour: buitenrand,
        # binnenrand en eventuele hoekradius. Nooit als profielnaad wegfilteren.
        return False
