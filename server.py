...
def _is_standard_profile_edge(
    ...
    """
    Return True only for ordinary stock-profile geometry.

    Important distinction:
    - long/seam edges belonging to the original rounded rectangle are ignored;
    - short longitudinal edges created by a notch/tab/cut-out remain FEATURES;
    - normal corner-radius arcs are ignored, even where a cut plane intersects
      the radius; drilled/other circular machining remains a feature.
    """
    ...
    # Rounded-corner arcs are part of the purchased profile, not laser cuts.
    # Identify them by their arc centre near a real profile corner. This also
    # catches radius arcs exposed by a notch at an intermediate Z position.
    if gt == "CIRCLE":
        try:
            r = float(edge.radius())
            center = np.array(edge.arcCenter().toTuple(), dtype=float) - c
            ...
            expected = corner_offset
            centre_tol = max(tol_xy, min(ow, oh) * 0.04)
            max_r = min(ow, oh) * 0.24

            if (
                1e-5 < r <= max_r
                and expected > 0.0
                and abs(dx - expected) <= centre_tol
                and abs(dy - expected) <= centre_tol
            ):
                return True
        except Exception:
            pass

        # Any other circle/ellipse/spline is genuine machining geometry.
        return False
    ...
    # True stock longitudinal edges live at a CORNER / radius tangent zone.
    # The previous code used `on_x OR on_y`, which accidentally classified
    # vertical walls of rectangular notches as stock edges. Require both
    # transverse coordinates to be near a real corner/tangent zone instead.
    if dz_span > max(dx_span, dy_span) * 5.0:
        corner_band = max(
            tol_xy * 1.35,
        ...
        if near_corner_x and near_corner_y:
            return True
        # Parallel-to-axis edge on a flat face away from a corner is exactly
        # the kind of vertical notch/hap edge that must remain highlighted.
        return False

    # Standard end perimeter is allowed only at the actual member ends.
    at_end = (
    ...
    if at_end and gt == "LINE":
        on_outer_x = all(abs(abs(x) - hpw) <= tol_xy for x in xs)
        on_outer_y = all(abs(abs(y) - hph) <= tol_xy for y in ys)

        # Straight perimeter segments of the normal tube end are stock edges.
        if on_outer_x or on_outer_y:
            return True

        # Inner wall end perimeter: infer wall offset from known corner centre.
        # We keep this conservative; unknown/interrupted lines stay features.
        if corner_offset > 0.0:
            inner_band = max(tol_xy * 1.35, corner_offset + tol_xy * 1.1)
            near_outer_shell_x = all((hpw - abs(x)) <= inner_band for x in xs)
            near_outer_shell_y = all((hph - abs(y)) <= inner_band for y in ys)
            if near_outer_shell_x and near_outer_shell_y:
                return True
