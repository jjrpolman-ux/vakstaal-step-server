...
STEP_MATERIAL_LENGTH_VERSION = 4  # longest real straight STEP side
...
def _physical_material_length_along_axis(
    ...
    """
    Vakstaal STEP materiaallengte v4.

    Bron van waarheid = de LANGSTE ECHTE rechte lengtezijde van de body.
    Niet: bounding box, hartlijn, totale projectie of analyzer-raw-length.

    Werkwijze:
    1. lees alle rechte topologische STEP-edges;
    2. de langste rechte edge bepaalt de kandidaat-lengterichting;
    3. alleen edges vrijwel parallel aan die richting mogen lengte-edges zijn;
    4. collineaire segmenten worden per fysieke zijlijn samengenomen;
    5. de grootste fysieke span van zo'n zijlijn wordt materiaallengte.

    Hierdoor geeft een schuin/verstek afgesneden koker de langste werkelijke
    zijde terug. Zijn twee of meer lange zijden gelijk, dan is de keuze
    ...
    for edge in solid.Edges():
        try:
            ...
            vec=q-p
            euclidean=float(np.linalg.norm(vec))
            if not math.isfinite(euclidean) or euclidean<=1e-6:
                continue
    ...
    if not straight:
        # Alleen voor exotische geometrie zonder rechte edges.
        axis, analyzer_length, method=_dominant_longitudinal_axis_and_length(solid)
        axis=np.array(axis,dtype=float)
        axis=axis/max(float(np.linalg.norm(axis)),1e-12)
        return max(0.0,float(analyzer_length or 0.0)),axis,f"{method}-no-straight-edge-fallback"

    # Dit is bewust NIET de analyzer-as. De langste echte rechte STEP-edge
    # bepaalt zelf de profielrichting.
    seed=max(straight,key=lambda item:item["len"])
    axis=np.array(seed["unit"],dtype=float)

    # Richtingsteken normaliseren voor stabiele diagnose.
    ...
    for item in straight:
        # 0.9995 ~= max 1,81° afwijking. Een echte extrusierand hoort vrijwel
        # exact parallel te zijn; verstek-/eindranden vallen hiermee af.
        parallel=abs(float(np.dot(item["unit"],axis)))
        if parallel<0.9995:
            continue
        ...
        # Gebruik de dwarse positie van beide eindpunten. Bij een echte
        # longitudinale rechte rand zijn die vrijwel gelijk.
        pu=float(np.dot(p,u)); pv=float(np.dot(p,v))
        qu=float(np.dot(q,u)); qv=float(np.dot(q,v))
        if math.hypot(qu-pu,qv-pv)>0.35:
            continue
    ...
    if not candidates:
        return float(seed["len"]),axis,"longest-straight-step-edge-v4"

    # Collineaire stukken op dezelfde fysieke hoek/zijde combineren.
    # Dit is nuttig wanneer een uitsparing één lange STEP-edge in delen splitst.
    line_tol=0.45
    groups=[]
    for seg in sorted(candidates,key=lambda x:x["edge_len"],reverse=True):
        found=None
        for group in groups:
            if abs(seg["u"]-group["u"])<=line_tol and abs(seg["v"]-group["v"])<=line_tol:
                found=group
                break
        if found is None:
            found={"u":seg["u"],"v":seg["v"],"intervals":[]}
            groups.append(found)
        found["intervals"].append((seg["lo"],seg["hi"]))
    ...
    if spans:
        return max(spans),axis,"longest-real-step-side-v4"
...
def _apply_physical_material_lengths(step_path: Path, result: dict) -> dict:
    ...
    result["material_length_definition"] = (
        "longest real straight longitudinal STEP side edge"
    )
...
def _assembly_cache_path(job_id: str) -> Path:
    return CACHE_DIR / job_id / "assembly_mesh_physical_cut_v63_real_side_length.json"
...
def dropbox_oauth_status():
    return {
        "ok":True,
        "app_key_configured":bool(DROPBOX_APP_KEY),
        "app_secret_configured":bool(DROPBOX_APP_SECRET),
        "redirect_uri_configured":bool(DROPBOX_REDIRECT_URI),
        "refresh_token_configured":bool(_dropbox_runtime_refresh_token),
        "ready_to_connect":bool(DROPBOX_APP_KEY and DROPBOX_APP_SECRET and DROPBOX_REDIRECT_URI),
    }


@app.get("/api/dropbox/oauth/start")
def dropbox_oauth_start():
    if not (DROPBOX_APP_KEY and DROPBOX_APP_SECRET and DROPBOX_REDIRECT_URI):
        raise HTTPException(status_code=503,detail=(
            "Dropbox OAuth is nog niet volledig ingesteld. "
            "DROPBOX_APP_KEY, DROPBOX_APP_SECRET en DROPBOX_REDIRECT_URI zijn verplicht."
        ))
...
def dropbox_oauth_callback(code: str="", state: str=""):
    global _dropbox_runtime_access_token, _dropbox_runtime_refresh_token
    expected=""
    try:
    ...
    if expected and state != expected:
        raise HTTPException(status_code=400,detail="Ongeldige Dropbox OAuth state.")
    if not code:
        raise HTTPException(status_code=400,detail="Dropbox gaf geen autorisatiecode terug.")
    body=urllib.parse.urlencode({
        "code":code,
    ...
    try:
        ...
        detail=exc.read().decode("utf-8",errors="replace")
        raise HTTPException(status_code=502,detail=f"Dropbox koppelen mislukt ({exc.code}): {detail[:700]}") from exc
    access=str(result.get("access_token") or "").strip()
    refresh=str(result.get("refresh_token") or "").strip()
    if not access:
        raise HTTPException(status_code=502,detail="Dropbox gaf geen access token terug.")
    ...
    if refresh:
        ...
        try:
            ...
            pass
    return HTMLResponse("""<!doctype html><html><head><meta charset="utf-8"><title>Dropbox gekoppeld</title></head>
<body style="font-family:Arial,sans-serif;padding:30px"><h2>Dropbox is gekoppeld</h2>
<p>Je kunt dit venster sluiten en teruggaan naar Vakstaal.</p>
<script>try{window.opener&&window.opener.postMessage({type:'vakstaal-dropbox-connected'},'*');}catch(e){}
setTimeout(()=>window.close(),900);</script></body></html>""")


@app.get("/api/dropbox/browser-root")
def dropbox_browser_root():
    """
    Hoogste map die de huidige Dropbox-app-token daadwerkelijk kan zien.
    Bij een Dropbox App Folder-token is dit de app-map; bij Full Dropbox is
    dit de echte Dropbox-root. De frontend doet nooit alsof hij hoger kan.
    """
    try:
        root_listing = _dropbox_list_folders("")
        return {
            "ok": True,
            "path": "",
            "display_name": "Dropbox start",
            "folder_count": len(root_listing.get("folders") or []),
            "note": (
                "Dit is het hoogste niveau waar de huidige Dropbox-koppeling toegang toe heeft."
            ),
        }
    except HTTPException as exc:
        return {
