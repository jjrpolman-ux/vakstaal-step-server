...
def _is_standard_profile_edge(
    ...
    if at_end:
        ...
        # Rounded rectangular tube corners at an end plane: every point lies
        # close to the outside envelope, but neither x nor y stays constant.
        # Allow a boundary band large enough for common R2-R8 tube radii while
        # keeping holes/slots in the face classified as real machining.
        boundary_band = max(1.25, min(8.0, min(ow, oh) * 0.12))
        near_outer_boundary = all(
            min(hpw - abs(x), hph - abs(y)) <= boundary_band
            and abs(x) <= hpw + tol_xy
            and abs(y) <= hph + tol_xy
            for x, y in zip(xs, ys)
        )
        if near_outer_boundary:
            return True
